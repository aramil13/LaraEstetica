...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api/salons' && method === 'POST') {
      ...
      const b = await readJson(request);
      const id = b.id || randomHex(16);
      await env.DB.prepare('INSERT INTO salons (id, name, address, phone, email, user_email) VALUES (?, ?, ?, ?, ?, ?)')
        .bind(id, b.name, b.address || null, b.phone || null, b.email || null, b.user_email || email).run();
